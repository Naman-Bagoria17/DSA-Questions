//means niche se hume har column ka last element dikhega
#include<bits/stdc++.h>
using namespace std;
class Node{
    public:
...
vector<int> BottomViewNodes(Node* root){
    vector<int> ans;
    if(root==nullptr)
        return ans;
    queue<pair<Node*,int>> q;
    map<int, int> m;//column->value
...
int main(){
    ...
    vector<int> result=BottomViewNodes(root);
    for(auto ele:result){
        cout << ele << " ";
    }
}